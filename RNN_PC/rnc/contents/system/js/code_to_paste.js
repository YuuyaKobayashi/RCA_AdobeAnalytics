...
if(typeof(s) != "undefined"){
	...
	var disp_id = window.location.pathname.split("/")[3],nList,isIE;
	//IE
	(navigator.userAgent.indexOf('msie') == -1)?isIE= 0:isIE = -3;
	...
	if(nList){
		var rqmtList,ss,bp,listType = getNListType(disp_id),d,apd;
		if(listType == "a"){
		//	rqmtList = $(".n_list").children(".JsRqmtInfo");
			rqmtList = document.getElementsByClassName("JsRqmtInfo");
		}else if(listType == "b"){
		//	rqmtList = $(".n_list_950").children(".JsRqmtInfo");
			rqmtList = document.getElementsByClassName("JsRqmtInfo");
		}else{
			rqmtList = $(".n_list_950").children("li[id*='rqmt']");
		}

		$('.n_list_header').each(function() {$(this).css('position','relative');});
		$('.n_list_header h2').each(function() {$(this).css('width','580px');});

		//Get RqmtPage
		function getRqmtDate(rqmtUrl,rqmtJQobj){
			$.ajax({
				...
				success:function(data){
					var date = $(data).find(".post_end").text();
					var dht;
					if(date){
						//Get Date Basetext
						var c =date.split("：")[1];
						...
						if(ss){
							//text Set 
							if(listType == "a"){
								d = $(rqmtJQobj).find(".fl.clr h2");
								dht = $(d).height();
								if(dht > 50){
									apd = '<span style="position: absolute; bottom: ' + String(11+isIE) + 'px;  width: 82px; height:16px; background: url(/rnc/contents/pub/inc_n_ichiran_950/img/n15_ico_n_upday.png) 0 0; overflow: hidden; text-indent: -9999%;'+ bp +'">' + ss + '</span>';
								}else if(dht <= 36){
									apd = '<span style="position: absolute; bottom: ' + String(18+isIE) + 'px;  width: 82px; height:16px; background: url(/rnc/contents/pub/inc_n_ichiran_950/img/n15_ico_n_upday.png) 0 0; overflow: hidden; text-indent: -9999%;'+ bp +'">' + ss + '</span>';								
								}else if(dht == 40){
									apd = '<span style="position: absolute; bottom: ' + String(15+isIE) + 'px;  width: 82px; height:16px; background: url(/rnc/contents/pub/inc_n_ichiran_950/img/n15_ico_n_upday.png) 0 0; overflow: hidden; text-indent: -9999%;'+ bp +'">' + ss + '</span>';
								}else{
									apd = '<span style="position: absolute; bottom: ' + String(14+isIE) + 'px;  width: 82px; height:16px; background: url(/rnc/contents/pub/inc_n_ichiran_950/img/n15_ico_n_upday.png) 0 0; overflow: hidden; text-indent: -9999%;'+ bp +'">' + ss + '</span>';
								}
							}else{
								d = $(rqmtJQobj).find(".leftcol.clr h2");
								apd = '<span style="position: absolute; bottom: 9px; right: 9px; width: 82px; height:16px; background: url(/rnc/contents/pub/inc_n_ichiran_950/img/n15_ico_n_upday.png) 0 0; overflow: hidden; text-indent: -9999%;'+ bp +'">' + ss + '</span>';
							}
							$(d).append(apd);
						}
					}
		...
		if(rqmtList.length>0){
			$(window).load(function(){
				//const
				var itrCnt = Math.floor(rqmtList.length/10);
				var lastCnt= (rqmtList.length) - (itrCnt*10);

				//get lazyAjax
				function getrqmtDOM1(s,e){
					for(i=s; i<=e; i++){
						getRqmtDate($(rqmtList[i]).find(".JsRqmtJobTypeName").children("a").attr("href"),$(rqmtList[i]));
					}
				}
				//Simple lazy-load
				if(itrCnt == 0){
					setTimeout(function(){getrqmtDOM1(0,lastCnt);},6000);
				}else if(itrCnt == 1){
					setTimeout(function(){getrqmtDOM1(0,9);},6000);
					setTimeout(function(){getrqmtDOM1(10,10+lastCnt);},7000);
				}else if(itrCnt == 2){
					setTimeout(function(){getrqmtDOM1(0,9);},6000);
					setTimeout(function(){getrqmtDOM1(10,19);},8000);
					setTimeout(function(){getrqmtDOM1(20,20+lastCnt);},10000);
				}else if(itrCnt == 3){
					setTimeout(function(){getrqmtDOM1(0,9);},6000);
					setTimeout(function(){getrqmtDOM1(10,19);},8000);
					setTimeout(function(){getrqmtDOM1(20,29);},10000);
					setTimeout(function(){getrqmtDOM1(30,30+lastCnt);},12000);
				}else if(itrCnt == 4){
					setTimeout(function(){getrqmtDOM1(0,9);},6000);
					setTimeout(function(){getrqmtDOM1(10,19);},8000);
					setTimeout(function(){getrqmtDOM1(20,29);},10000);
					setTimeout(function(){getrqmtDOM1(30,39);},12000);
					setTimeout(function(){getrqmtDOM1(40,40+lastCnt);},14000);
				}else if(itrCnt == 5){
					setTimeout(function(){getrqmtDOM1(0,9);},6000);
					setTimeout(function(){getrqmtDOM1(10,19);},8000);
					setTimeout(function(){getrqmtDOM1(20,29);},10000);
					setTimeout(function(){getrqmtDOM1(30,39);},12000);
					setTimeout(function(){getrqmtDOM1(40,49);},14000);
					setTimeout(function(){getrqmtDOM1(50,50);},16000);
				}
			});
		}
	}
